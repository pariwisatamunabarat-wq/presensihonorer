             ...
             <div class="grid grid-cols-1 gap-6 mb-6">
                 <div>
                     ...
                     <div class="bg-gray-100 p-4 rounded-lg">
                         ...
                         {{-- Tambahkan informasi hari yang diizinkan --}}
                         <p><strong>Hari Kerja : </strong>{{ implode(', ', $schedule->getAllowedDaysNames()) }}</p>
                         @if($schedule->is_wfa)
                             <p class="text-green-500"><strong>Status : </strong>WFA</p>
                         @else
                             <p><strong>Status : </strong>WFO</p>
                         @endif
                         {{-- Tampilkan status apakah hari ini diizinkan --}}
                         @if(!$schedule->isAllowedToday())
                             <p class="text-red-500"><strong>⚠️ Hari ini bukan hari kerja Anda</strong></p>
                         @endif
                     </div>
                 ...
                 <div>
                    ...
                    @endif
                     <form class="row g-3 mt-4" wire:submit="store" enctype="multipart/form-data">
                         {{-- Disable tombol jika bukan hari kerja --}}
{{-- Lanjutan resources/views/livewire/presensi.blade.php --}}
                         @if($schedule->isAllowedToday())
                             ...
                             @endif
                         @else
                             <button type="button" disabled class="px-4 py-2 bg-gray-400 text-white rounded cursor-not-allowed">Presensi Tidak Tersedia</button>
                             <p class="text-sm text-red-500 mt-2">Anda hanya dapat melakukan presensi pada: {{ implode(', ', $schedule->getAllowedDaysNames()) }}</p>
                         @endif
                     </form>
                 </div>
    ...
    <script>
         ...
         let component;
         let marker;
         const isAllowedToday = {{$schedule->isAllowedToday() ? 'true' : 'false'}};
         ...
         function tagLocation() {
             // Cek apakah hari ini diizinkan sebelum melakukan tag location
             if (!isAllowedToday) {
                 alert('Anda tidak dapat melakukan presensi hari ini.');
                 return;
             }
